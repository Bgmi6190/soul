...
def disapprove_user(message):
    if not is_user_admin(message.from_user.id, message.chat.id):
        bot.send_message(message.chat.id, "*You are not authorized to use this command*.Please contact to admin :- @MR_SID_001", parse_mode='Markdown')
        return
